...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // --- كود لقطة الشاشة المضاف ---
    const screenshotBtn = document.getElementById('screenshot-btn');
    if (screenshotBtn) {
        screenshotBtn.addEventListener('click', async () => {
            const topButtons = document.getElementById('top-right-buttons');
            showMessage('جارٍ تحضير لقطة شاشة 4K...', 'info');
            
            // إخفاء الأزرار قبل التقاط اللقطة
            if (topButtons) topButtons.style.display = 'none';

            try {
                // حساب مقياس 4K
                const targetWidth = 3840; // عرض 4K UHD
                const sourceWidth = document.body.scrollWidth;
                const scale = targetWidth / sourceWidth;

                // استخدام html2canvas لالتقاط body بالكامل
                const canvas = await html2canvas(document.body, {
                    scale: scale,
                    useCORS: true,
                    allowTaint: true,
                    backgroundColor: '#1a202c' // لون خلفية التطبيق (bg-dark)
                });
                ...
                // إنشاء اسم ملف ديناميكي بالتاريخ والوقت
                const date = new Date();
                const timestamp = `${date.getFullYear()}${(date.getMonth() + 1).toString().padStart(2, '0')}${date.getDate().toString().padStart(2, '0')}_${date.getHours().toString().padStart(2, '0')}${date.getMinutes().toString().padStart(2, '0')}`;
                link.download = `MasterKey_Screenshot_4K_${timestamp}.png`;
                ...
                showMessage('تم التقاط لقطة الشاشة بنجاح!', 'success');

            } catch (error) {
                console.error('فشل التقاط لقطة الشاشة:', error);
                showMessage('فشل التقاط لقطة الشاشة. حاول مرة أخرى.', 'error');
            } finally {
                // إظهار الأزرار مرة أخرى سواء نجحت العملية أو فشلت
                if (topButtons) topButtons.style.display = 'flex';
            }
        });
    }
    // --- نهاية كود لقطة الشاشة ---
